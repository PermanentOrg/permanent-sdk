...
const shareLinkVoAccessRoleToAccessRole = (accessRole?: string | null): AccessRole | undefined => (
...
export const shareLinkVoToShareLink = (shareLinkVo: ShareLinkVo): ShareLink => {
  ...
  const expiresAt = shareLinkVo.expiresDT !== undefined && shareLinkVo.expiresDT !== null
    ? new Date(formatTimestampAsUtc(shareLinkVo.expiresDT))
    : undefined;
  const status = shareLinkVoStatusToStatus(shareLinkVo.status);
  const defaultAccessRole = shareLinkVoAccessRoleToAccessRole(shareLinkVo.defaultAccessRole);
  return {
