...
const stelaRecordTypeToArchiveRecordType = (recordType: string): ArchiveRecordType => (
...
export const stelaRecordToArchiveRecord = (stelaRecord: StelaRecord): ArchiveRecord => {
  const createdAt = new Date(formatTimestampAsUtc(stelaRecord.createdAt));
  const updatedAt = stelaRecord.updatedAt === null || stelaRecord.updatedAt === undefined
    ? createdAt
    : new Date(formatTimestampAsUtc(stelaRecord.updatedAt));
  const displayDate = stelaRecord.displayDate === null || stelaRecord.displayDate === undefined
    ? null
  ...
  const files = stelaRecord.files.map((stelaFile) => stelaFileToFile(stelaFile));
  return {
